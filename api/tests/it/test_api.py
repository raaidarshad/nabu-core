...
def test_search_returns_empty_if_no_similar_articles():
    body = {"url": "https://www.nytimes.com/live/2022/02/24/world/russia-attacks-ukraine", "client_id": "test"}
    response = client.post(ENDPOINT, json=body)
    assert response.status_code == 200
...
def test_search_returns_404_if_article_not_found():
    body = {"url": "https://www.fake.com/this/does-not-exist", "client_id": "test"}
    response = client.post(ENDPOINT, json=body)
